...
class ElmerAdapter(Adapter):
    ...
    def generate(self, output_dir: Path) -> tuple[Path, ...]:
        ...
        lines = [
            ...
            "End",
            "",
            "! CAVEAT ON THE LATERAL BOUNDARIES, recorded 2026-09-20.",
            "! Emitter, Collector AND SideWalls are all no-slip, so this domain is CLOSED ON THREE",
            "! SIDES with only TopFarField open. The side walls stand many gap-widths away from a",
            "! wire whose radius is orders of magnitude smaller, so they are a far-field truncation",
            "! rather than physical surfaces, and no-slip is the wrong condition",
            "! for them once anything drives a flow. It is harmless HERE only because there is no",
            "! body force, so the solve is trivially zero everywhere and no air has to enter.",
            "! The moment a body force is added, air would be driven against a sealed box through a",
            "! single opening, and a thruster needs an inlet. Choosing the lateral condition is a",
            "! physics decision with a citation requirement and belongs to the fluid domain, so it",
            "! is stated here rather than guessed at. Revisit BEFORE adding the EHD body force.",
